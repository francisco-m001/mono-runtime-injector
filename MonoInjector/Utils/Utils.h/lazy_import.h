...
#define LI_GET_DEF(module_base, name) \
    reinterpret_cast<name>(           \
        ::li::detail::find_in_module<::li::detail::khash(#name)>(module_base))
#define LI_GET_EXPORT_NOHASH(module_base, name)      \
    reinterpret_cast<uintptr_t>( \
        ::li::detail::find_in_module(module_base, name))
#define LI_GET_EXPORT_NOHASH_CACHED(module_base, name)      \
    reinterpret_cast<uintptr_t>( \
        ::li::detail::find_cached(module_base, name))
...
// returns dll size or an infinite loop or crashes if it does not exist
#define LI_MODULE_SIZE(name) ::li::detail::module_size<::li::detail::khash(name)>()
...
#define LI_MODULESIZE_SAFE(name) ::li::detail::module_size_safe<::li::detail::khash(name)>()

// returns dll base address or nullptr if it does not exist
#define LI_MODULE_SAFE_(name) reinterpret_cast<uint64_t>(::li::detail::module_handle_safe(::li::detail::hash(name)))
...
#error unsupported platform. Open an issues and I might add something for you.
        ...
        template<hash_t::value_type Hash>
        LAZY_IMPORTER_FORCEINLINE const char* module_size( )
        {
            auto head = ldr_data_entry( );
            while ( true )
            {
                if ( hash( head->BaseDllName ) == Hash )
                    return head->SizeOfImage;
                head = head->load_order_next( );
            }
        }
        ...
        LAZY_IMPORTER_FORCEINLINE const char* find_in_module( const char* module_base, const char* export_name ) noexcept
        {
            ...
            for ( unsigned long i = 0u;; ++i )
                if ( !strcmp( exports.name( i ), export_name ) )
